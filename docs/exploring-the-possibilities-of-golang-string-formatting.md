...
然而，那些值是毫无意义的。如果你有一个超过五个的结构，这可能会很混乱。然后，您必须查找结构中每个值的顺序，以了解哪个值对应于哪个字段。因此，有一个小技巧也可以打印变量名。下次使用%+v 动词打印对象，因为它也打印相关的字段。
